...
Declaring significant only 1 time point that comes from a null hypothesis constitutes a type 1 error.
...
2. Use the Bonferroni procedure for the decision of the hypothesis $H_{(1)}$ while considering the hypothesis $H_{(1)},\dots,H_{(m)}$ ($\tilde p_{(1)} =,p_{(1)}$).
...
- Interpreting the univariate cluster depth tests will increase the FWER.
- We use Troendle (min-p + step-wise arguments) as it is powerful when the tests have different univariate distributions.


### Interpretation

- We take a $\alpha_{\text{FWER}}$ when we declare: "We found at least one cluster that begins no later than time $t_0$."
