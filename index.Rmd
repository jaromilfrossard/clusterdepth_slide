...
.center[
.large[600 ms at 512Hz = 614 Time-Points!]
]
...
#### Definition
- $\text{FWER}_{\text{H}0} = \text{Prob}(\text{At least 1 type 1 Error})$.
...
- Under H0, any statistics computed on the permutated data has the same probability to be observed.
...
1. As $E<1$, the influence of the length of the clusters is smaller. 
2. It **decreases the risk of false positive** at the border of true effects.
...
`r emo::ji("idea")` What is the distribution of the 1st point of the cluster?

`r emo::ji("idea")` What is the distribution of the 2nd point of the cluster?
...
- Interpreting the univariate cluster depth tests increases the FWER.
- We use **Troendle** (min-p + step-wise argument) as it is powerful when the tests have different univariate distributions.
